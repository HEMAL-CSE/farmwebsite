...
const EarthwormLabourPayment = () => {
    ...
    const [edit_labour, setEdit_labour] = useState('')
    ...
    const getData = () => {
        axios.get('http://68.178.163.174:5010/vermi_compost/earthworm_labour_payment').then(res => {
            setData(res.data)
        })
    }
    ...
    useEffect(() => {
        ...
        axios.get(`http://68.178.163.174:5010/vermi_compost/earthworm_labour`)
            .then(res => {
                setLabours(res.data)
            })
    ...
    const addData = (e) => {
        ...
        axios.post('http://68.178.163.174:5010/vermi_compost/earthworm_labour_payment/add', {
            labour_id: labour,
            amount,
            date,
        }).then(res => {
    ...
    const editData = (e, id) => {
        ...
        axios.put(`http://68.178.163.174:5010/vermi_compost/earthworm_labour_payment/edit?id=${id}`, {
            labour_id: edit_labour,
            amount: edit_amount,
            date: edit_date
        })
            .then(res => {
    ...
    const deleteData = (e, id) => {
        ...
        if (window.confirm('Do you want to delete this?')) {
            axios.delete(`http://68.178.163.174:5010/vermi_compost/earthworm_labour_payment/delete?id=${id}`)
                .then(res => {
                    toast('Deleted')
                    getData()
        ...
        <Vermicompost>
            <div className='details'>
                        ...
                        <div className="col-lg-6">
                            <div className="d-flex align-items-center justify-content-center">
                                <a href="index.html" className="navbar-brand ms-lg-5">
                                    <h1 className="m-2 display-4 text-success2"><span className="text-success2">Earthworm Labour</span> Payment</h1>
                                </a>
                            </div>
                        </div>
                ...
                <form>
                    ...
                    <select value={labour} onChange={e => {
                ...
                >
                    ...
                    <label>Select Labour:</label>
                    <select value={edit_labour} onChange={e => {
                        setEdit_labour(e.target.value)
                    }} className='select' >
                        <option >Select</option>
                        {
                            labours.map(shed => (
...
export default EarthwormLabourPayment
...
const DoctorBeef = () => {
    ...
    useEffect(() => {
      let user_id = localStorage.getItem('user_id')
          ...
          axios.get(`http://68.178.163.174:5010/doctors/beef/doctor?user_id=${user_id}`).then(res => {
              // console.log(res.data);
  
              if (res.data.length > 0) {
                  let results = res.data.reduce((obj, item) => {
                      let id = item.cattle_id
  ...
  const handleSubmit = e => {
    ...
    axios.put(`http://68.178.163.174:5010/doctors/beef/treatment?doctor_id=${user_id}`, {
      treatment,
    }).then(res => {
      toast('Submitted')
      setIsOpen(false)

...
const DoctorDairy = () => {
    ...
    useEffect(() => {
      let user_id = localStorage.getItem('user_id')
          ...
          axios.get(`http://68.178.163.174:5010/doctors/dairy/doctor?user_id=${user_id}`).then(res => {
  ...
  const handleSubmit = e => {
    ...
    axios.put(`http://68.178.163.174:5010/doctors/dairy/treatment?doctor_id=${user_id}`, {
      treatment,
    }).then(res => {
      toast('Submitted')
      setIsOpen(false)

...
const BiogasEnvironmentInfo = () => {
  ...
  const getData = () => {
    axios.get('http://68.178.163.174:5010/biogas/environment').then(res => {
      setData(res.data)
    })
  }
  ...
  const addData = (e) => {
    ...
    axios.post('http://68.178.163.174:5010/biogas/environment/add', {
      pressure,
      temperature,
  ...
  const editData = (e, id) => {
    ...
    axios.put(`http://68.178.163.174:5010/biogas/environment/edit?id=${id}`, {
  ...
  const deleteData = (e, id) => {
    ...
    if (window.confirm('Do you want to delete this?')) {
      axios.delete(`http://68.178.163.174:5010/biogas/environment/delete?id=${id}`)
        .then(res => {
          toast('Deleted')
          getData()
